...
mean.steps <- floor(mean(data.by.day$steps))
median.steps <- floor(median(data.by.day$steps))
options("scipen"=100, "digits"=4)
```

We can see here by inspection that the mean number of steps per day is **`r mean.steps`** and the median is **`r median.steps`**, and can also see that each of these matches nicely with what we could have inferred from the histogram.
...
```{r echo=FALSE, message=FALSE}
mean.filled.steps <- floor(mean(data.filled.by.day$steps))
median.filled.steps <- floor(median(data.filled.by.day$steps))
```

We can see here by inspection that the mean number of steps per day is now **`r mean.filled.steps`** where before it was **`r mean.steps`**. The median is now **`r median.filled.steps`**, where before it was **`r median.steps`**.
